...
export class GlobalNotificationsComponent implements OnInit {
  ...
  getSystemAlertsAndCheckNotificationAccepted(notification: GlobalNotification) {
    ...
    this.dashboardService.getSystemAlerts().subscribe({
      next: summary => {
        this.systemAlertSummary = summary;

        if (this.systemAlertSummary.acceptable_alerts) {
          Object.keys(this.systemAlertSummary.acceptable_alerts).forEach(alertKey => {
            const alert = this.systemAlertSummary.acceptable_alerts[alertKey];
  ...
  private generateSystemAlertNotifications(): void {
    this.handleSystemAlertNotifications(this.systemAlertSummary.acceptable_alerts, true);
    this.handleSystemAlertNotifications(this.systemAlertSummary.unacceptable_alerts, false);
  }

  private handleSystemAlertNotifications(summary: {
    [name: string]: SystemAlerts;
  }, acceptable: boolean): void {
    Object.keys(summary).forEach(k => {
      const alerts: SystemAlerts = summary[k];
      ...
      alerts.data.forEach(a => this.addSystemAlertGlobalNotification(k, a, link, acceptable));
    });
  }

  private addSystemAlertGlobalNotification(name: string, alert: SystemAlert, link: string, acceptable: boolean): void {
    const notification: GlobalNotification = {
      ...
      link: link,
      labelClass: this.getLabelClassFromSystemAlertSeverity(alert.severity),
      acceptable: true,
    ...
    if(acceptable) {
      notification.accepted = this.systemAlertSummary.accepted_alerts
      ? this.systemAlertSummary.accepted_alerts.includes(alert.id)
      : false;
    }
